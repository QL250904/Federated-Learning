...
def start_rf_client():
    print("="*50)
    print(" MÁY TRẠM CLIENT 1 (KIỂU TRUYỀN THỐNG TRONG AI)")
    print(" Cơ chế: Nộp toàn bộ file gốc cho Server (Không bảo mật)")
    print("="*50)
    
    # Load dataset cá nhân (chia một nửa)
    ...
    try:
        ...
        print(f"[Client 1] Đang truyền {len(X)} hình ảnh GỐC (~{len(encoded_data)/1024/1024:.1f} MB) cho Server...")
        client_socket.sendall(encoded_data + b"END_OF_TRANSMISSION")
        print("[Client 1] Nộp thành công. Dừng tiến trình!")
